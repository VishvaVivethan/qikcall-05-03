...
import { Box, Grid, Typography, TextField,Button,Snackbar,Alert,Checkbox } from '@mui/material';
...
const descriptionElementRef = React.useRef(null);
  ...
  const validateStep = () => {
    let newErrors = {};
      ...
      if (!alterphonenumber) {
        newErrors.alterphonenumber = "Alterphonenumber is required";
      } else if(!alterphonenumber.length===10) {
          newErrors.alterphonenumber = "Phone number must be 10 Digits";
      }
      if (!pannumber) {
        newErrors.pannumber = "Pan number is required";
      } else if(!pannumber.length===10) {
          newErrors.pannumber = "Pan number must be 10 Digits";
      }
      if (!aadhar) {
  ...
  return (
    ...
    <>
   <NavBar/>
<Grid mt={5} mb={5} container justifyContent="space-between" sx={{ height: 'auto',backgroundColor: '#2d2f6b' }}>
      {/* Left Section */}
      <Grid 
        item 
        xs={12} md={5.5} 
      ...
      >
        <Typography mt={3} variant="h5" gutterBottom sx={{ fontWeight: 'bold' }}>
...
<Grid  container direction="row" justifyContent="flex-start" alignItems='center'>
  ...
  alignItems="center">
        <Button 
        sx={{ backgroundColor: "#2d2859", fontSize: "15px", width: "200px","&:hover": { backgroundColor: "#4b487e" }  }} 
        variant='contained'
        onClick={()=>{
          ...
          if (!checked) {
            showError('Accept terms and conditions');
            return;
          }
...
<>
<NavBar/>
<Grid   container justifyContent="space-between" sx={{ height: 'auto' }}>
      {/* Left Section */}
      <Grid 
        item 
        xs={12} md={5.5} 
  ...
  >
    <IoMdHome onClick={handlehome} size={24} /> {/* Adjust size as needed */}
  </Box>
