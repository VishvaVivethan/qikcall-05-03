...
import { Box, Grid, Typography, TextField,Container,FormHelperText, Button,CardMedia,Card,Snackbar,Alert,InputLabel,Select,FormControl,MenuItem,Toolbar,AppBar,IconButton, Link,Tooltip } from '@mui/material';
...
import Updateanimation from "../../assets/image/restart.gif"
...
const fetchUserById = async () => {
  try {
    ...
    setChecked(result.data.isapprove || false); // Set checkbox value if applicable
  } catch (error) {
...
const updateUser = async () => {
  ...
  const validateStep = () => {
    ...
    if (activeStep === 0) {
      ...
      if (!formData.phonenumber) {
        newErrors.formData.phonenumber = "Phonenumber is required";
      } else if(!formData.phonenumber.length===10) {
          newErrors.formData.phonenumber = "Phone number must be 10 Digits";
      }
  ...
  const [updateopen, setUpdaeOpen] = useState(false);
...
<Dialog open={updateopen} onClose={handleUpdateColse}>
      <DialogTitle>Update completed</DialogTitle>
      <DialogContent sx={{width:"300px",alignItems:"center",justifyContent:"center"}}>
        <Grid container direction="column" alignItems="center" justifyContent="center">
        <img 
          src={Updateanimation}
          alt="Done GIF" 
          onClick={handleUpdateColse}
