...
// import React, { useEffect, useState } from 'react';
// import { Card, Container, CardMedia, Typography, Grid, Box, Rating } from '@mui/material';
// import { Favorite } from '@mui/icons-material'; // Import the heart icon
// import './style.css';
// import NavBar from '../navbar';
// import Footer from '../footer';
// import Cookies from 'js-cookie';
...
//         try {
//             console.log('Deleting item with id:', id); // Print the id of the item being deleted
    
//             const response = await fetch('/api/delete_wishlist', {
//                 method: 'DELETE',
//                 headers: {
//                     'Content-Type': 'application/json',
//                     'Authorization': `Bearer ${Cookies.get('token')}`, // Ensure token is sent
//                 },
...
//                         data.map((item, index) => (
//                             item && item.addimages && item.addimages.length > 0 ? (
//                                 <Grid item xs={isMobile ? 6 : 6} key={index}>
//                                     <Card sx={{ backgroundColor: "#f7f4cd", border: "1px solid black", padding: 2 }}>
//                                         <Grid container direction="row" justifyContent="space-between" alignItems="center">
//                                             <Grid item xs={5}>
//                                                 <CardMedia
...
function Wishlist() {
    ...
    return (
        <>
            ...
            <Container sx={{ marginTop: "20px", padding: isMobile ? "0 8px" : "0 16px" }}>
                <Grid mb={5} container direction="row" justifyContent="flex-start" alignItems="center" spacing={2}>
                    {data && Array.isArray(data) && data.length > 0 ? (
                        data.map((item, index) => (
                            item && item.addimages && item.addimages.length > 0 ? (
                                <Grid item xs={isMobile ? 12 : 6} key={index}>
                                    <Card sx={{ backgroundColor: "#f7f4cd", border: "1px solid black", padding: 2 }}>
                                        <Grid container direction={isMobile ? "column" : "row"} justifyContent="space-between" alignItems="center">
                                            <Grid item xs={isMobile ? 12 : 5}>
                                                <CardMedia
                                                    component="img"
                                                    className={isMobile ? "product-image-mob" : "product-image"}
                                                    src={item.addimages[0]}
                                                    alt="Product Image"
                                                />
                                            </Grid>
                                            <Grid item xs={isMobile ? 12 : 5} container direction="column" justifyContent="space-evenly" alignItems={isMobile ? "center" : "flex-start"}>
                                                <Typography id="text-head">{item.servicename}</Typography>
                                                <Box className="mt-1">
                                                    <Typography id="text">Address:</Typography>
                                                    <Typography>{`${item.addressline1}, ${item.addressline2}, ${item.area}, ${item.city}, ${item.state}`}</Typography>
                                            ...
                                            <Grid item xs={isMobile ? 12 : 4} container justifyContent="center" alignItems="center">
                                                <Favorite
                                                    style={{ color: 'red', cursor: 'pointer' }}
                                                    onClick={() => handleDelete(item._id)}
                                                />
                        ...
                        <Grid mb={4} mt={4} container direction="row" justifyContent="center" alignItems="center">
                            <Card sx={{ backgroundColor: "#f7f4cd", border: "1px solid black", padding: 2, width: isMobile ? "100%" : "500px" }}>
                                <Typography variant="h6" align="center">No items in your wishlist</Typography>
                            </Card>
                        </Grid>
                    )}
