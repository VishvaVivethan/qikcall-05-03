...
const getWishlistById = (userid) => {
  try {
    ...
    fetch(`/api/wishlistdata?id=${userid}`, requestOptions)
      ...
      .then((result) => {
        if (result.status_code === 200) {
          console.log("Wishlist data", result.data.data);
          const wishlistData = result.data.data;
          setDatabyId(wishlistData);
          if (wishlistData && wishlistData.wishlist && wishlistData.wishlist.storeId === cdata?._id) {
...
const saveWishlist = async () => {
  try {
    ...
    const response = await fetch("http://localhost:2525/api/wishlist", requestOptions);
...
useEffect(() => {
  ...
  const ratingsToShow = showMore ? data?.ratings : data[0]?.ratings?.slice(0, 10);
  ...
  const filteredData = alldata.filter(data => {
    ...
    const serviceTypeMatches = Array.isArray(data.servicetype) && 
                               ...
                               data.servicetype.some(type => data.servicetype.includes(type));
    if (serviceTypeMatches) {
        return true;
    }
    console.log(`No match for servicetype in ${data.servicetype}`);
    console.log(`Match for servicetype in ${data.servicetype}`);
    return false;
});
