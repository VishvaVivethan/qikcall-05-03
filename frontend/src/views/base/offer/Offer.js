...
const getOfferById =(offerid)=>{
 try{
  ...
  fetch(`/api/offerdata?id=${offerid}`, requestOptions)
  .then(async (response) => {
    if (response.status === 200 || response.status === 400) {
      return { status_code: response.status, data: await response.json() };
    } else {
...
const updateOffer = async (offerid) => {
  try {
    ...
    const finalImageUrls = imageUrls.length > 0 ? imageUrls : addimages; // 'existingImages' should hold previously stored images
    ...
    const response = await fetch(`/api/offer_update?id=${offerid}`, requestOptions);
...
const offerDelete =(offerid)=>{
  try{
    ...
    fetch(`/api/delete_offer?id=${offerid}`, requestOptions)
    .then(async (response) => {
      if (response.status === 200 || response.status === 400) {
          return { status_code: response.status, data: await response.json() };
      } else {
...
fetch("/api/offer", requestOptions)
.then(async (response) => {
  if (response.status === 200 || response.status === 400) {
      return { status_code: response.status, data: await response.json() };
  } else {
          ...
          <Grid container direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Grid item xs={6} container direction="row" justifyContent="flex-end" alignItems="center">
              <Button onClick={handleCreateOpen} color="inherit">
                <AddCircleIcon sx={{ mr: 0.5 }} /> Add Offers
              </Button>
            </Grid>
          </Grid>
                  ...
                  <Grid container direction="row" justifyContent="space-evenly" alignItems="center">
                    <Grid container rowSpacing={2} columnSpacing={{ xs: 1 }} sx={{ display: 'flex', justifyContent: 'space-around' }}>
                      ...
                      <Grid item sm={12} md={6} lg={4} container direction="row" justifyContent="center" alignItems="center">
                        ...
                        <TextField sx={{ marginTop: '30px', width: '100%' }} value={cstorename} onChange={(e) => setCStorename(e.target.value)} placeholder="Store Name" variant="outlined" />
