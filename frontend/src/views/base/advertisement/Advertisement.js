  ...
  try{
    ...
    .then((result) => {
      ...
      setAdtitle(result.data.data.adtitle || '');
        ...
        setContact(result.data.data.contactnumber || '');
...
const updateAdvertise = async (advertiseid) => {
  try {
    ...
    const finalImageUrls = imageUrls.length > 0 ? imageUrls : addimages; // 'existingImages' holds previously stored images
    ...
    const raw = JSON.stringify({
      ...
      addimages: finalImageUrls // Use the final images (new or existing)
    });
    ...
    const response = await fetch(`/api/advertise_update?id=${advertiseid}`, requestOptions);
    const result = await response.json();
    
    if (response.status === 200 || response.status === 400) {
      console.log(result, "Advertise updated successfully");
      getAdvertise();
    } else {
      console.log("Unexpected error occurred.");
    }
  } catch (error) {
    console.log("Error updating advertise:", error);
  }
};
...
const [cadcategory, setCAdcategory] = useState('');
...
const raw = JSON.stringify({
  "adtitle": cadtitle,
  "addescription": caddescription,
  "adcategory": cadcategory,
  "location": clocation,
...
const advertiseDelete =(advertiseid)=>{
  try{
    const requestOptions = {
      method: "DELETE",
      redirect: "follow"
    ...
    fetch(`/api/delete_advertise?id=${advertiseid}`, requestOptions)
     .then(async (response) => {
  if (response.status === 200 || response.status === 400) {
      return { status_code: response.status, data: await response.json() };
  } else {
...
console.log(checked,"log")
  ...
  const handleDeleteOpen = (advertiseid) => {
    setDelete(true);
    getAdvetiseById(advertiseid)
    console.log(advertiseid,"delete id")
  };
 ...
 const handleRegister = () => {
          ...
          <Grid container direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Grid item xs={6} container direction="row" justifyContent="flex-end" alignItems="center">
              <Button onClick={handleCreateOpen} color="inherit">
                <AddCircleIcon sx={{ mr: 0.5 }} /> Add Advertisement
              </Button>
            </Grid>
          </Grid>
                  ...
                  <Grid container direction="row" justifyContent="space-evenly" alignItems="center">
                    <Grid container rowSpacing={2} columnSpacing={{ xs: 1 }} sx={{ display: 'flex', justifyContent: 'space-around' }}>
                      <Grid item sm={12} md={6} lg={4} container direction="row" justifyContent="center" alignItems="center">
                        <TextField sx={{ marginTop: '30px', width: '100%' }} value={cadtitle} onChange={(e) => setCAdtitle(e.target.value)} placeholder="AD Title" variant="outlined" />
                        <TextField sx={{ marginTop: '30px', width: '100%' }} value={caddescription} onChange={(e) => setCAddescription(e.target.value)} placeholder="AD Description" variant="outlined" />
                        ...
                        <TextField sx={{ marginTop: '30px', width: '100%' }} value={ccontactnumber} onChange={(e) => setContactnumber(e.target.value)} placeholder="Contact Number" variant="outlined" />
                      </Grid>
                      <Grid item sm={12} md={6} lg={4} container direction="row" justifyContent="center" alignItems="center">
  ...
  />
            ...
            <DialogActions>
              <Button onClick={handleCreateClose} sx={{ backgroundColor: '#FC4343', color: '#fff' }}>Cancel</Button>
              <Button onClick={handleRegister} sx={{ backgroundColor: '#2D2859', color: '#fff' }} >
                Register
              </Button>
            </DialogActions>
          </Dialog>
  ...
  <Grid item xs={4}>
  {Array.isArray(item.service) && item.service[0] ? (
  ...
  />
      ...
      >
        ...
        <DialogActions>
          ...
          <Button variant='contained' onClick={() => deleteAdvertise(advertise._id)} autoFocus sx={{ backgroundColor: 'green', color: '#fff','&:hover': {backgroundColor: 'green' }}}>
           Yes
          </Button>
          </Grid>
        </DialogActions>
